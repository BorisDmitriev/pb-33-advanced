...
describe("1. `uppercase` Function", () => {
  test("`uppercase` should accept a string and convert the first letter of each word of the string to uppercase", () => {
    ...
    expect(uppercase("web development tutorial")).toEqual("Web Development Tutorial");
  });
});
...
describe("2. Longest word within a string", () => {
  test("`findLongestWord` should accept a string and return the longest word within that string", () => {
    ...
    expect(findLongestWord('Web Development Tutorial')).toEqual('Development');
  });
});
...
describe("3. Longest Country Name", () => {
  test("`longestCountryName` should accept an array of country names and return the longest country name", () => {
    ...
    expect(longestCountryName(["Australia", "Germany", "Democratic Republic of Sao Tome and Principe"])).toEqual(expect.stringMatching(/Democratic Republic of Sao Tome and Principe/i));
  });
});

describe("4. Count Vowels", () => {
  test("`countWovels` should accept a string and return the number of vowels it contains", () => {
    const solution = rewire("../index");
    const countWovels = solution.__get__("countWovels");
    expect(countWovels("Australia")).toBe(5);
  });
});
